...
A Docker-first B2B merchandise portal built on Medusa v2 and Next.js.

## First deployment with Portainer

1. Copy `.env.example` to `.env` and replace every `CHANGE_ME` value with a long,
   unique secret. In Portainer, the same keys can be entered as stack variables.
2. Set `STOREFRONT_URL`, `BACKEND_URL`, and `ADMIN_URL` to the HTTPS addresses your
   reverse proxy will expose.
3. Deploy `docker-compose.yml` as a stack.
4. Open the client shell at port `8000/portal`, the staff shell at `8000/admin`, and
   Medusa Admin at port `9000/app`.
5. Create the first Medusa administrator inside the backend container:

   ```text
   pnpm --filter @dtc/backend exec medusa user -e you@example.com -p a-strong-password
   ```

6. In Medusa Admin, create a publishable API key and assign it to a sales channel.
   Set `NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY` and rebuild the storefront service before
   enabling catalog-backed storefront routes.

MinIO's administration console is exposed on port `9001`. PostgreSQL, Redis,
Typesense, and the MinIO API are internal-only by default.

## Local validation

Use the included environment template when validating the stack:
...
See `docs/ARCHITECTURE.md` for system boundaries and `docs/PROJECT_STATUS.md` for the
current milestone.

## Upstream starter notes
