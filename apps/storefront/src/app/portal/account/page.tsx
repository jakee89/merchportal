...
export default async function AccountPage() {
  ...
  return <div className={styles.page} style={{ "--client-color": me.organization.primary_color } as React.CSSProperties}>
    ...
    <main className={styles.main}><div className={styles.sectionTitle}><div><span className={styles.eyebrow}>Private client catalog</span><h1>{me.organization.name}</h1></div></div>
      {catalog.products.length ? <div className={styles.grid}>{catalog.products.map((product) => <article className={styles.card} key={product.id}>{product.image_url ? <img className={styles.productImage} src={`${backend}${product.image_url}`} alt="" /> : <div className={styles.productVisual}>M</div>}<h3>{product.name}</h3><p className={styles.muted}>{product.description}</p>{product.price_eur !== undefined && <p className={styles.price}>EUR {product.price_eur.toFixed(2)}</p>}<p className={styles.status}>{product.stock_quantity !== undefined ? `${product.stock_quantity} available` : "Availability on request"}{product.sku ? ` · Code ${product.sku}` : ""}</p></article>)}</div> : <div className={styles.empty}><h2>Catalog is ready for its first import</h2><p>Ask a staff member to open MerchPortal in Admin and run a catalog update.</p></div>}
    </main>
  </div>
}
