...
type Product = { id: string; sku?: string; name: string; description?: string; image_url?: string; price_eur?: number; stock_quantity?: number; category?: string; colors: string[]; lead_time?: string; sustainable: boolean; print_methods: string[] }
type Facets = { categories: string[]; colors: string[]; lead_times: string[]; print_methods: string[] }
  ...
  return <div className={styles.page} style={{ "--client-color": me.organization.primary_color } as React.CSSProperties}>
    ...
    <main className={styles.main}><div className={styles.sectionTitle}><div><span className={styles.eyebrow}>Private client catalog</span><h1>{me.organization.name}</h1></div></div>
      <form className={styles.filters} method="get">
        <label className={styles.searchField}>Search<input name="q" defaultValue={chosen("q")} placeholder="Product name or code" /></label>
        <label>Category<select name="category" defaultValue={chosen("category")}><option value="">All categories</option>{catalog.facets.categories.map((item) => <option key={item}>{item}</option>)}</select></label>
        <label>Colour<select name="color" defaultValue={chosen("color")}><option value="">All colours</option>{catalog.facets.colors.map((item) => <option key={item}>{item}</option>)}</select></label>
        <label>Minimum price<input type="number" min="0" step="0.01" name="min_price" defaultValue={chosen("min_price")} /></label>
        <label>Maximum price<input type="number" min="0" step="0.01" name="max_price" defaultValue={chosen("max_price")} /></label>
        <label>Lead time<select name="lead_time" defaultValue={chosen("lead_time")}><option value="">Any lead time</option>{catalog.facets.lead_times.map((item) => <option key={item}>{item}</option>)}</select></label>
        <label>Print method<select name="print_method" defaultValue={chosen("print_method")}><option value="">Any print method</option>{catalog.facets.print_methods.map((item) => <option key={item}>{item}</option>)}</select></label>
        <label className={styles.check}><input type="checkbox" name="in_stock" value="true" defaultChecked={selected.in_stock === "true"} />In stock</label>
        <label className={styles.check}><input type="checkbox" name="sustainable" value="true" defaultChecked={selected.sustainable === "true"} />Sustainable</label>
