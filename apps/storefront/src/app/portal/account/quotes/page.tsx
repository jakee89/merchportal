...
export type QuoteItem = { id: string; product_id: string; product_name: string; image_url?: string; color: string; quantity: number; decorations: Array<{ method_name: string; position_name: string }>; artwork_filename?: string; estimated_total: number | null; quote_required: boolean }
...
export default async function QuotePage() {
  ...
  return <div className={styles.page}>
    ...
    <main className={styles.productMain}><h1>Quote cart</h1><p className={styles.helper}>Review your configured products, then send one request for a final price.</p><QuoteCart cart={cart} history={history} /></main>
  </div>
}
