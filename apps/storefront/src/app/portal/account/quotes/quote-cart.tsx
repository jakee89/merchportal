...
function Item({ item, remove, backend }: { item: QuoteItem; remove?: (id: string) => void; backend: string }) {
  return <article className={styles.quoteItem}>
    ...
    <div className={styles.quoteItemDetails}><Link href={`/portal/account/products/${item.product_id}${item.sku ? `?sku=${encodeURIComponent(item.sku)}` : ""}`}><strong>{item.product_name}</strong></Link><p>{item.color}{item.sku ? ` · ${item.sku}` : ""} · {item.quantity.toLocaleString()} units</p>{item.base_unit_price && <small>Plain product €{new Intl.NumberFormat("en-IE", { minimumFractionDigits: 2, maximumFractionDigits: 6 }).format(item.base_unit_price)} per unit</small>}{item.decorations.map((line, index) => <div className={styles.quoteDecoration} key={index}>{(line.position_image_url || item.image_url) && <ProductImage src={mediaUrl(backend, line.position_image_url)} fallbackSrc={mediaUrl(backend, item.image_url)} name={`${line.position_name} print guide`} />}<span><strong>{line.method_name} · {line.position_name}</strong><small>{[line.print_width_mm && line.print_height_mm ? `${line.print_width_mm} × ${line.print_height_mm} mm` : "", line.print_colours ? `${line.print_colours} print colour${line.print_colours === 1 ? "" : "s"}` : ""].filter(Boolean).join(" · ")}</small><small>{line.price_pending || line.unit_price_eur === null || line.unit_price_eur === undefined ? "Print price to confirm" : `€${new Intl.NumberFormat("en-IE", { minimumFractionDigits: 2, maximumFractionDigits: 6 }).format(line.unit_price_eur)} per unit${line.setup_price_eur ? ` + €${line.setup_price_eur.toFixed(2)} setup` : ""}`}</small></span></div>)}{item.artwork_filename && <small>Artwork: {item.artwork_filename}</small>}</div>
