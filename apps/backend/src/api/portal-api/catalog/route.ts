...
export async function GET(req: AuthenticatedMedusaRequest, res: MedusaResponse) {
  ...
  if (cached && cached.expires > Date.now()) {
    ...
    const [sources, markup] = await Promise.all([nativeProducts.length ? service.listPublishedProductSources({ product_id: nativeProducts.map((product: any) => product.id) }) : [], resolveMarkup(service, membership[0].organization_id)])
    ...
    catalogCache.set(cacheKey, { expires: Date.now() + 60_000, products: safeProducts, facets })
  }
  ...
  res.json({ products, facets, total: filtered.length, page, page_size: pageSize, page_count: pageCount })
}
