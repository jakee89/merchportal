...
import { buyerProfile } from "../../../modules/merchportal/buyer-profile"
...
export async function GET(req: AuthenticatedMedusaRequest, res: MedusaResponse) {
  ...
  const buyerDetails = await buyerProfile(req.scope, service, actorId, membership.organization_id)
...
export async function POST(req: AuthenticatedMedusaRequest<{ note?: string }>, res: MedusaResponse) {
  const { service, membership, actorId } = await customerQuoteContext(req, true)
  const note = String(req.body?.note || "").trim().slice(0, 2000)
  const details = validateQuoteDetails(await buyerProfile(req.scope, service, actorId, membership.organization_id))
