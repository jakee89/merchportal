...
type SupplierCode = "stricker" | "midocean" | "aodaci" | "makito"
...
export function supplierCredentialStatus(code: SupplierCode, configuration?: SupplierConfiguration | null) {
  return Boolean(configuration?.encrypted_api_key || (code === "stricker" ? process.env.STRICKER_ACCESS_KEY : code === "midocean" ? process.env.MIDOCEAN_API_KEY : code === "makito" ? process.env.MAKITO_CREDENTIALS : process.env.AODACI_ACCESS_KEY))
}
...
export function resolveSupplierCredential(code: SupplierCode, configuration?: SupplierConfiguration | null) {
  ...
  return code === "stricker" ? process.env.STRICKER_ACCESS_KEY : code === "midocean" ? process.env.MIDOCEAN_API_KEY : code === "makito" ? process.env.MAKITO_CREDENTIALS : process.env.AODACI_ACCESS_KEY
}
